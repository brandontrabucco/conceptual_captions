...
import captionkit
from captionkit.show_and_tell_cell import ShowAndTellCell
...
tf.flags.DEFINE_string("tfrecord_file_pattern", "../train_tfrecords/?????.tfrecord", "Pattern of the TFRecord files.")
...
if __name__ == "__main__":
    ...
    sat = ShowAndTellCell(1024)
    captioner = ImageCaptioner(sat, vocab, np.random.normal(0, 0.001, [len(reverse_vocab), 1024]))
    cnn = ResNet()
    logits, ids = captioner(lengths=tf.reduce_sum(dataset["indicator"], axis=1), 
        mean_image_features=tf.squeeze(tf.squeeze(cnn(dataset["image"]), 1), 1), seq_inputs=dataset["input_seq"])
    ...
    with tf.Session() as sess:
        ...
        captioner_saver.save(sess, "./ckpts/model.ckpt", global_step=global_step)
        last_save = time.time()
